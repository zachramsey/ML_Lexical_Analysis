...
class Net(nn.Module):
    def __init__(self, w_len):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv1d(4*w_len, 16*w_len, 3, 1, 1),
            nn.ReLU(),
            nn.MaxPool1d(2, 2, 1) )
        self.conv2 = nn.Sequential(
            nn.Conv1d(16*w_len, 64*w_len, 3, 1, 1),
            nn.ReLU(),
            nn.MaxPool1d(2, 2, 1) )
        self.flatten = nn.Flatten()
        self.dense1 = nn.Sequential(nn.Linear(64*w_len, 16*w_len), nn.ReLU(), nn.Dropout(0.5))
        self.dense2 = nn.Sequential(nn.Linear(16*w_len, 4*w_len), nn.ReLU(), nn.Dropout(0.5))
