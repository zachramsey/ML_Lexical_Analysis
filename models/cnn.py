...
class Net(nn.Module):
    def __init__(self, w_len):
        ...
        # self.wGRU = nn.Sequential(nn.GRU(w_len, w_len, 2, batch_first=True, bidirectional=True))
        self.w_conv = nn.Sequential(
            nn.Conv1d(w_len, 2*w_len, 3, 1, 1),
            nn.ReLU(),
            nn.MaxPool1d(2, 2, 1) )
        ...
        self.conv3 = nn.Sequential(
            nn.Conv1d(16*w_len, 32*w_len, 3, 1, 1),
            nn.ReLU(),
            nn.MaxPool1d(2, 2, 1) )
        self.flatten = nn.Flatten()
        self.dense1 = nn.Sequential(nn.Linear(32*w_len, 16*w_len), nn.ReLU(), nn.Dropout(0.5))
        self.dense2 = nn.Sequential(nn.Linear(16*w_len, 8*w_len), nn.ReLU(), nn.Dropout(0.5))
        self.dense3 = nn.Sequential(nn.Linear(8*w_len, 4*w_len), nn.ReLU(), nn.Dropout(0.5))
